...
function filterProducts() {
    ...
    products.forEach(product => {
        ...
        // Verifique se o produto atende aos critérios de filtragem
        ...
        // Exibe ou oculta o produto com uma animação suave
        if (matchesSearch && matchesAuthor && matchesGenre && matchesPrice && matchesNewRelease && matchesOnSale) {
            product.classList.remove('hidden');
            setTimeout(() => { product.style.display = 'block'; }, 300); // Mostrar após a animação
        } else {
            product.classList.add('hidden');
            setTimeout(() => { product.style.display = 'none'; }, 300); // Esconder após a animação
        }
    });
}
...
// Resetar filtros com animação suave
document.getElementById('reset-button').addEventListener('click', function () {
    ...
    // Desmarcar todos os checkboxes de gênero
    document.querySelectorAll('input[name="genre"]').forEach(input => input.checked = false);

    // Reexibe todos os produtos com animação
    const products = document.querySelectorAll('.product-card');
    products.forEach(product => {
        product.classList.remove('hidden');
